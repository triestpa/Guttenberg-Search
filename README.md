...
In this tutorial, we'll walk through setting up our own full-text search application (of an admittedly lesser complexity than the systems in the questions above).  Our example app will provide a UI and API to search through the complete texts of 100 literary classics such as *Peter Pan*, *Frankenstein*, and *The Communist Manifesto*.
...
1. Use regular expressions (check out [this post](https://blog.patricktriest.com/you-should-learn-regex/) for a primer on using regex) to parse the book title and author.  
1. Identify the start and end of the book content, by matching on the all-caps "Project Guttenberg" header and footer.  
1. Extract the book text content.
1. Split each paragraph into its own array.
1. Clean up the text and remove blank lines.
